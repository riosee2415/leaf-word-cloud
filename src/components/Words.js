...
class Words extends React.Component {
  ...
  _get = async () => {
    await fetch(`${databaseURL}/words.json`)
      .then((res) => {
        if (res.status != 200) {
  ...
  _post = async (word) => {
    return await fetch(`${databaseURL}/words.json`, {
      method: "POST",
  ...
  _delete = async (id) => {
    return await fetch(`${databaseURL}/words/${id}.json`, {
      method: "DELETE",
    })
      .then((res) => {
        if (res.status != 200) {
  ...
  handleDialogToggle = () => this.setState({ isDialog: !this.state.isDialog });
